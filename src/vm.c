...
static void runtimeError(const char *format, ...) {
  ...
  for (int i = vm.frameCount - 1; i >= 0; i--) {
    ...
    fprintf(stderr, "[line %d] in ",
            getLine(&frame->closure->function->chunk.lines, instruction));
...
static bool callValue(Value callee, int argCount) {
  if (IS_OBJ(callee)) {
    ...
    case OBJ_NATIVE: {
      ...
      Value result =
          native(argCount, &vm.stack->values[vm.stack->count] - argCount);
  ...
  for (;;) {
    ...
    case OP_CLOSURE: {
      ...
      for (int i = 0; i < closure->upvalueCount; i++) {
        ...
        if (isLocal) {
          closure->upvalues[i] =
              captureUpvalue(&vm.stack->values[frame->stack_pointer + index]);
        } else {
    ...
    case OP_RETURN: {
      Value result = pop();
      closeUpvalues(&vm.stack->values[frame->stack_pointer]);
      vm.frameCount--;
      if (vm.frameCount == 0) {
        pop();
        return INTERPRET_OK;
